...
def main(robot: pyniryo.NiryoRobot) -> None:
    ...
    while True:
        try:
            ...
            movement_times.append(time.time() - before)
            print("Movement time: " + str(movement_times[-1]))
            print("Close/Open Gripper every: " + str(sum(movement_times) / len(movement_times)))
